...
for (b in 1:nrow(freqt)){
  for (g in 1:nrow(TXcity)){
    
    city1<-TXcity[g,1]
    city2<-as.character(freqt[b,1])
    #icome<-as.integer(TxCity4[g,4])
    if(city2==city1){
      newRow2<-data.frame(City_Name=city1, income=TXcity[g,4])
...
freqt<-merge(freqt, incomes, by="City_Name")

#Clean up witespace on income
freqt$income<-trimws(freqt$income)

#rename
colnames(freqt)[colnames(freqt)=="Income"]<-"income"

#Assign groupnamesfor data
for (i in 1:nrow(freqt)){
  f <-as.numeric(freqt$income[i])
  if(f < 15000 & !is.na(f)){
    freqt[i,4]<-"Poverty"
    freqt[i,5]<-15000
    freqt[i,6]<-"$0-$14,999" 
...
for (i in 1:nrow(freqt)){
  f <-as.numeric(freqt$income[i])
  if(f>=15000 & f<25000 & !is.na(f)){
    freqt[i,4]<-"Poverty"
    freqt[i,5]<-15000
    freqt[i,6]<-"$15,000 - $24,999" 
    freqt[i,7]<-(f/100)
...
for (i in 1:nrow(freqt)){
  f <-as.numeric(freqt$income[i])
  if(f>=25000 & f<35000 & !is.na(f)){
    freqt[i,4]<-"Low Income"
    freqt[i,5]<-25000
    freqt[i,6]<-"$15,000 - $24,999" 
    freqt[i,7]<-(f/100)
...
for (i in 1:nrow(freqt)){
  f <-as.numeric(freqt$income[i])
  if(f>=35000 & f<50000 & !is.na(f)){
    freqt[i,4]<-"Middle Income"
    freqt[i,5]<-50000
    freqt[i,6]<-"$35,000 - $49,999" 
    freqt[i,7]<-(f/100)
...
for (i in 1:nrow(freqt)){
  f <-as.numeric(freqt$income[i])
  if(f>=50000 & f<76000 & !is.na(f)){
    freqt[i,4]<-"Middle Income"
    freqt[i,5]<-76600
    freqt[i,6]<-"$50,000 - $75,999"
    freqt[i,7]<-(f/100)
...
for (i in 1:nrow(freqt)){
  f <-as.numeric(freqt$income[i])
  if(f>=76000 & f<100000 & !is.na(f)){
    freqt[i,4]<-"High Income"
    freqt[i,5]<-100000
    freqt[i,6]<-"$76,000 - $99,999"   
    freqt[i,7]<-(f/100)
...
for (i in 1:nrow(freqt)){
  f <-as.numeric(freqt$income[i])
  if(f>=100000 & f<150000 & !is.na(f)){
    freqt[i,4]<-"High Income"
    freqt[i,5]<-150000
    freqt[i,6]<-"$100,000 - $149,999" 
    freqt[i,7]<-(f/100)
...
for (i in 1:nrow(freqt)){
  f <-as.numeric(freqt$income[i])
  if(f>=150000 & f<200000 & !is.na(f)){
    freqt[i,4]<-"High Income"
    freqt[i,5]<-200000
    freqt[i,6]<-"$150,000 - $199,999" 
    freqt[i,7]<-(f/100)
...
colnames(freqt)[colnames(freqt)=="V4"]<-"Economic Status"
colnames(freqt)[colnames(freqt)=="V5"]<-"Income Limit"
colnames(freqt)[colnames(freqt)=="V6"]<-"Income Bracket"
colnames(freqt)[colnames(freqt)=="V7"]<-"Divided Income"

#This one works
attach(freqt)
ggplot(freqt, aes(as.integer(`Divided Income`),Freq)) +
...
ggplot(freqt, aes(x=`Economic Status`, y = Freq, fill = `Economic Status`)) +
  geom_boxplot()
